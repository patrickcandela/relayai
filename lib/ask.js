...
const API_BASE = 'https://relayai-landing.vercel.app';
...
function fetchProfile(binId) {
  return new Promise((resolve, reject) => {
    const url = new URL(`${API_BASE}/api/get-profile?id=${binId}`);
...
module.exports = async function ask(code) {
  ...
  let friend;
  try {
    friend = await fetchProfile(binId);
  } catch (err) {
